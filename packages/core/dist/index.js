...
function PopChild({ children, isPresent, anchorX, root }) {
  ...
  return jsx(PopChildMeasure, { isPresent, childRef: ref, sizeRef: size, children: React2.cloneElement(children, { ref }) });
}
...
function useVisualElement(Component3, visualState, props, createVisualElement, ProjectionNodeConstructor) {
  ...
  const wantsHandoff = useRef4(Boolean(optimisedAppearId) && !window.MotionHandoffIsComplete?.(optimisedAppearId) && window.MotionHasOptimisedAnimation?.(optimisedAppearId));
  useIsomorphicLayoutEffect(() => {
    if (!visualElement)
      return;
...
var HaloButton = React4.forwardRef(({
  variant = "primary",
  size = "md",
  loading = false,
  leftIcon,
  ...
  return /* @__PURE__ */ React4.createElement(
    ...
    loading ? /* @__PURE__ */ React4.createElement("span", { className: "animate-pulse" }, "Loading...") : /* @__PURE__ */ React4.createElement(React4.Fragment, null, leftIcon && /* @__PURE__ */ React4.createElement("span", { className: "inline-flex items-center", "aria-hidden": true }, leftIcon), content && /* @__PURE__ */ React4.createElement("span", null, content), rightIcon && /* @__PURE__ */ React4.createElement("span", { className: "inline-flex items-center", "aria-hidden": true }, rightIcon))
  );
});
...
function useStore(api, selector = identity) {
  const slice = React5.useSyncExternalStore(
    api.subscribe,
    React5.useCallback(() => selector(api.getState()), [api, selector]),
    React5.useCallback(() => selector(api.getInitialState()), [api, selector])
  );
  React5.useDebugValue(slice);
  return slice;
}
...
var FocusRing = ({ inset = false, radius = 6, ...rest }) => {
  return /* @__PURE__ */ React6.createElement("div", { "data-halo-focus": true, "data-inset": inset ? "true" : "false", "data-radius": radius, className: "halo-focus-ring", ...rest });
};
...
var ScrollArea = ({ height = 240, children }) => {
  ...
  return /* @__PURE__ */ React7.createElement("div", { className: "halo-scroll-area", "data-halo-scroll": true, "data-maxh": styleVar }, children);
};
...
import { useMemo as useMemo7, useState as useState3 } from "react";
...
function TimeMachine() {
  ...
  const timeline = useMemo7(() => events.filter((e) => /(run|edit|capsule)\./.test(e.type)).sort((a, b) => a.ts - b.ts), [events]);
...
import { useEffect as useEffect6, useRef as useRef5, useState as useState5 } from "react";
function WizardStream({ runId }) {
  const [status, setStatus] = useState5(null);
  const [done, setDone] = useState5(false);
  const liveRef = useRef5(null);
  useEffect6(() => {
...
function PromptForm({ defaultProductSlug = "", defaultType = "component" }) {
  const [name, setName] = useState6(defaultProductSlug);
  const [goal, setGoal] = useState6("");
  const [type, setType] = useState6(defaultType);
  const [runId, setRunId] = useState6(null);
