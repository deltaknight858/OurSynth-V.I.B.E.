...
import type { GraphContext, MemoryEdge, MemoryNode } from '../types.js';
...
export function MindmapView({ loadContext, rootNodeId, depth = 2, width = 800, height = 600 }: MindmapViewProps) {
  ...
  return (
    <svg width={width} height={height} className="border" data-border-color="var(--border-color, #ccc)">
      {edges.map((e) => {
      ...
      {nodes.map((n) => {
        ...
        return (
          <g key={n.id} transform={`translate(${pos.x},${pos.y})`} className="cursor-pointer">
